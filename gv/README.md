...
**本目录是组员提供的原始 VAE 参考包，不作为项目正式训练入口。**

参考代码中的已知问题已经在项目模块中修正和重新实现：

- `../../src/gv_eval/vae.py`：与冻结配置一致的 GRU-VAE；
- `../../src/gv_eval/generation.py`：训练、验证、早停、恢复和生成流程；
- `../../experiments/train_generator.py`：项目训练入口；
- `../../experiments/generate_candidates.py`：候选生成入口；
- `../../tests/test_vae.py`：模型与微型端到端测试。

本目录继续保留以便审计代码来源，不应与正式词表、checkpoint 或生成结果混用。
