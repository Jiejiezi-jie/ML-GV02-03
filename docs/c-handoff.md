...
原始 C 交接产物未包含额外结构域和跨膜拓扑。后续对**同一冻结 B 快照**分别完成
[本地 Pfam 域架构复核](../results/gv02_03_c_domains_member_a_v1/README.md)与
[本地跨膜预警检查](../results/gv02_03_c_tm_member_a_v1/README.md)。Pfam 扫描未检出非目标家族命中，
30 条候选出现多段 PF00741 命中、需人工复核；跨膜预测有 7 条警告，均在原排除池。
两项结果彼此独立，均未改写 B 的 QC 或候选池，也不把模型预测当作功能/结构验证。

连续疏水片段只是启发式警告，不等同于跨膜预测。`qc_pass_extended.fasta` 不是主池；
无显著 Pfam 命中或无跨膜警告也不能证明候选不存在其他结构域或膜相关特征。
原交接不输出最终候选推荐，也不声明完整 C 生物学验证已完成。

三批结果的逐条合并与下游使用口径见 [C→下游统一复核交接](c-final-handoff.md)；
本页及原 `report.md` 仍记录原交接运行的历史范围。
