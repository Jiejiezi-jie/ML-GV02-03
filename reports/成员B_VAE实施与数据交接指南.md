...
当前实际划分为 31 簇：训练 277 条/16 簇、验证 33 条/9 簇、测试 36 条/6 簇（合计 346）。三组平均长度分别为 108.8、108.8 和 111.7 aa。划分在不拆分任何 70% identity 同源簇的前提下，同时约束序列数、簇数和总长度；不使用家族标签以外的下游结果。集合仍较小，验证/测试指标应附样本量，不作过强泛化结论。
